...
bool rLocalSimulator::Start() {
  ...
  // CC Eastbound stops
  Stop * stop_CC_EB_1 = new Stop(0, 44.972392, -93.243774);  // West Bank
  Stop * stop_CC_EB_2 = new Stop(1, 44.973580, -93.235071);  // Student Union
  Stop * stop_CC_EB_3 = new Stop(2, 44.975392, -93.226632);  // Oak/Washington
  ...
  // TODO(3081W staff):
  // is this always true? If so, we may want to
  // reduce the length of probs to_char_type
...
bool rLocalSimulator::Update() {
  ...
  for (std::vector<int>::iterator it = time_since_last_bus_generation_.begin();
    ...
    if (0 >= (*bus_gen_timing_iter)) {
      ...
      active_buses_.push_back(new BusBase(std::to_string((*bus_counter_iter)),
