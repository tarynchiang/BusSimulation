...
void GetBussesCommand::execute(MyWebServerSession* session,
    ...
    for (int i = 0; i < static_cast<int>(busses.size()); i++) {
        ...
        cStruct["r"] = picojson::value(busses[i].color.r);
        cStruct["g"] = picojson::value(busses[i].color.g);
        cStruct["b"] = picojson::value(busses[i].color.b);
        cStruct["a"] = picojson::value(busses[i].color.a);
...
class BusWebObserver : public IObserver<BusData>{
 public:
  ...
  void Notify(BusData* info) { // This is normally called Update,
                               // but we call it Notify as per the lab writeup
    picojson::object data;
...
class StopWebObserver : public IObserver<StopData>{
 public:
  ...
  void Notify(StopData* info) { // This is normally called Update,
                               // but we call it Notify as per the lab writeup
    picojson::object data;
    data["command"] = picojson::value("observe");
    std::stringstream ss;
    ...
    ss << "  * Position: (" << info->position.x << ","
                                << info->position.y << ")\n";
    ss << "  * Number of people waiting: " << info->num_people << "\n";
    data["text"] = picojson::value(ss.str());
    picojson::value ret(data);
    session->sendJSON(ret);
  }
...
void AddListenerCommand::execute(MyWebServerSession* session,
    ...
    // mySim->AddListener(&id, new StopWebObserver(session));
}

AddListenerStopCommand::AddListenerStopCommand(VisualizationSimulator* sim)
                                                        : mySim(sim) {}

void AddListenerStopCommand::execute(MyWebServerSession* session,
        picojson::value& command, MyWebServerSessionState* state) {
    mySim->ClearListeners();
    std::cout << "starting AddListenerStopCommand::execute" << std::endl;
    std::string id = command.get<picojson::object>()["id"].get<std::string>();
    std::cout << id << std::endl;
