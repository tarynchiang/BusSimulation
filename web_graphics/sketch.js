        ...
        socket.onmessage =function got_packet(msg) {
            ...
            if (data.command == "updateBusses") {
                ...
                for (let i = 0; i < data.busses.length; i++) {
                    ...
                    //add color component
                    r = data.busses[i].color.r;
                    g = data.busses[i].color.g;
                    b = data.busses[i].color.b;
                    a = data.busses[i].color.a;
...
function mapClick(event) {
    ...
    for (let i = 0; i < stops.length; i++) {
        ...
        // If we are over the stops
        if (abs(mouseX - pos.x) < 25 && abs(mouseY - pos.y) < 15) {
            console.log("hit Stop!!!");
            socket.send(JSON.stringify({command: "listen_stop", id: stops[i].id}));
            return;
        }
    }
    console.log("stop miss!!!");
}
